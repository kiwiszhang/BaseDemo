...
class CoreDataViewController: SuperViewController {
    ...
    private lazy var selectPhoto = UILabel().text("创建Person").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
    ...
    private lazy var saveToSandbox = UILabel().text("创建Pet关联人").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
        guard let self = self else {return}
        let persons = manager.fetchPersons()
    ...
    private lazy var saveToiCloud = UILabel().text("更新Person").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
        guard let self = self else {return}
    ...
    private lazy var iCloudData = UILabel().text("更新Pet").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
        guard let self = self else {return}
        let persons = manager.fetchPersons()
    ...
    private lazy var deleteFromiCloud = UILabel().text("删除Person").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
        guard let self = self else {return}
    ...
    private lazy var updateiCloudData = UILabel().text("删除Pet").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
        guard let self = self else {return}
...
class CoreDataTableViewViewCell: SuperTableViewCell {
    ...
    private lazy var petLab = UILabel().text("").color(.systemRed).hnFont(size: 12.w).centerAligned().backgroundColor(.white)
