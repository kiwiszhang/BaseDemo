...
class iCloudDocumentViewController: SuperViewController {
    ...
    // MARK: - =====================lazy load=======================
    private lazy var selectPhoto = UILabel().text("选择照片").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
    ...
    private lazy var saveToSandbox = UILabel().text("保存到沙盒").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
        guard let self = self else {return}
    ...
    private lazy var saveToiCloud = UILabel().text("保存到iCloud").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
        guard let self = self else {return}
        let imageName = UserDefaults.standard.string(forKey: "imageName")
    ...
    private lazy var updateiCloudData = UILabel().text("列出所有文件").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
        guard let self = self else {return}
    ...
    private lazy var importiCloud = UILabel().text("导入本地文件").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
        guard let self = self else {return}
    ...
    private lazy var exportiCloudData = UILabel().text("导出文件到本地").hnFont(size: 14.h, weight: .bold).backgroundColor(.systemCyan).color(.red).centerAligned().onTap {
        [weak self] in
        guard let self = self else {return}
